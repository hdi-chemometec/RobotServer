...
def getProtocols():
    ...
    if(IP_ADDRESS == "False"):
        return Response(json.dumps({'type': 'ERROR', 'message':'No connection to robot'}), status=200, mimetype=contentType)
...
def postRun():
    ...
    try:
        ...
        return Response(json.dumps({'type': 'ERROR', 'message':'No id found in body request'}), status=200, mimetype=contentType)
    ...
    if(robotRequest.status_code == 201):
        ...
        return Response(json.dumps({'type': 'ERROR', 'message':'Run was not created'}), status=200, mimetype=contentType)
...
def getCurrentRun():
    ...
    if (len(runRequest.keys()) == 0):
        return Response(json.dumps({'type': 'ERROR', 'message':'No runs found'}), status=200, mimetype=contentType)
...
def getRunStatus():
    ...
    try:
        ...
        return Response(json.dumps({'type': 'ERROR', 'message':'No IP to robot found'}), status=200, mimetype=contentType)
    ...
    if(robotRequest.status_code == 200):
        ...
        return Response(json.dumps({'type': 'ERROR', 'message':'{error}'.format(error=robotRequest)}), status=200, mimetype=contentType)
...
def postRunAction():
    ...
    try:
        ...
        return Response(json.dumps({'type': 'ERROR', 'message':'No command found'}), status=200, mimetype=contentType)
    except requests.exceptions.ConnectionError:
        return Response(json.dumps({'type': 'ERROR', 'message':'No connection to robot'}), status=200, mimetype=contentType)

    if(getRunStatus() == "succeeded" or getRunStatus() == "stopped"):
        return Response(json.dumps({'type': 'ERROR', 'message':'Run has already been executed'}), status=200, mimetype=contentType)
    ...
    currentRunRequest = getCurrentRun()
    if(len(currentRunRequest) == 0):
        return Response(json.dumps({'type': 'ERROR', 'message':'No current run'}), status=200, mimetype=contentType)
    ...
    if(robotRequest.status_code == 201):
        ...
        return Response(json.dumps({'type': 'ERROR', 'message':'{error}'.format(error=robotRequest.reason)}), status=200, mimetype=contentType)